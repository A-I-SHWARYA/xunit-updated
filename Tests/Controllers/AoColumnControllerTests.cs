...
namespace Tests.Controllers
{
    public class AoColumnControllerTests
    {
        ...
        [Fact]
        public async Task AddColumn_ShouldReturnBadRequest_WhenForeignKeyIsInvalid()
        {
            // Arrange
            var column = fixture.Create<Aocolumn>();

            // Set TableId to a value that doesn't correspond to an existing table
            column.TableId = Guid.NewGuid(); // You can use any random Guid here

            var expectedExceptionMessage = "Foreign key constraint violation: Table does not exist.";
            columnInterface.Setup(c => c.AddColumn(column)).Throws(new Exception(expectedExceptionMessage));

            // Act
            var result = await columnController.AddColumn(column) as BadRequestObjectResult;
            ...
            errorMessage.Should().Be(expectedExceptionMessage);
        ...
        [Fact]
        public async Task GetColumnsByType_ShouldReturnOk_WhenSuccess()
        {
            // Arrange
            var name = fixture.Create<string>();
            var expectedColumns = fixture.Create<IEnumerable<Aocolumn>>();
            columnInterface.Setup(c => c.GetColumnsByType(name)).ReturnsAsync(expectedColumns);

            // Act
            var result = await columnController.GetColumnsByType(name) as OkObjectResult;

            // Assert
            result.Should().NotBeNull();
            result.Should().BeAssignableTo<OkObjectResult>();

            var actualColumns = result.Value as IEnumerable<Aocolumn>;
            actualColumns.Should().NotBeNull();
