...
namespace Tests.Controllers
{
    public class AoTableControllerTest
    {
        ...
        [Fact]
        public async Task CheckName_ShouldReturnOk_WhenSuccess()
        {
            // Arrange
            var name = fixture.Create<string>();
            var expectedTables = fixture.Create<IEnumerable<Aotable>>();
            tableInterface.Setup(c => c.CheckName(name)).ReturnsAsync(expectedTables);

            // Act
            var result = await tableController.CheckName(name) as OkObjectResult;

            // Assert
            result.Should().NotBeNull();
            result.Should().BeAssignableTo<OkObjectResult>();

            var actualTables = result.Value as IEnumerable<Aotable>;
            actualTables.Should().NotBeNull();

            // Check if the actualTables and expectedTables are the same
        ...
        [Fact]
        public async Task GetByType_ShouldReturnOk_WhenSuccess()
        {
            // Arrange
            var type = fixture.Create<string>();
            var expectedTables = fixture.Create<IEnumerable<Aotable>>();
            tableInterface.Setup(c => c.GetByType(type)).ReturnsAsync(expectedTables);

            // Act
            var result = await tableController.GetByType(type) as OkObjectResult;
